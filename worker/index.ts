...
/**
 * 🔴 BE-002 — MUTASİYA MARŞRUTLARINDA AVTORİZASİYA BƏYANI MƏCBURİDİR.
 *
 * Auditin tapıntısı: 213 `auth: true` marşrutunda avtorizasiya yalnız handler-in
 * İÇİNDƏ idi. İzlənən 18 handler-in hamısı düzgün qapı işlədirdi — yəni boşluq
 * mövcud kodda deyil, NÖVBƏTİ marşrutdadır. Yeni endpoint əlavə edən developer
 * `auth: true` yazıb handler-də yoxlamanı unutsa, sistem səssizcə açıq qalır və
 * bunu nə tip, nə test, nə lint tutur.
 *
 * Həll `rl`-in artıq işlətdiyi naxışdır: sahə buraxıla bilməz, istisna AÇIQ
 * yazılmalıdır. `tsc` sınır — yəni unutmaq mümkün deyil.
 *
 * ⚠ İKİ DƏYƏR, HƏR İKİSİ MAŞINLA YOXLANILIR (`test/route-authz.test.ts`):
 *
 *   'handler' — `auth: true` var, deklarativ `perm` yoxdur; avtorizasiya
 *               handler-in içindədir (`gate()`, `scopedWhere`, `msgScopeGuard`,
 *               `taskFor`, `assertCanModerate` …). Bu, BƏYANDIR: kod baxışında
 *               "bəs yoxlama haradadır?" sualını məcbur edir.
 *   'public'  — `auth` YOXDUR və bu qəsdlidir (auth axını, OAuth, newsletter,
 *               contact). Siyahı testdə SABİTLƏNİB: yeni publik mutasiya əlavə
 *               etmək testi də dəyişməyi tələb edir — yəni nəzərdən keçirilir.
 *
 * ⚠ ÜÇÜNCÜ KATEQORİYA ('self') SINANDI VƏ ATILDI. Fikir belə idi: yolunda id
 *   tutma qrupu olmayan marşrut yalnız çağıranın öz sətrinə toxunur. YANLIŞDIR —
 *   `POST /api/ws/bulk`, `/api/messages/forward`, `/api/notifications/bulk`
 *   id-ləri GÖVDƏDƏ daşıyır. Bu, məhz auditin BE-002-də tənqid etdiyi ad-əsaslı
 *   yalançı siqnal olardı: nişan "təhlükəsiz" deyər, kod isə başqa şey edər.
 *   Yalan bəyan bəyanın olmamasından PİSDİR.
 */
type Ownership = 'handler' | 'public';

/** Gövdəni dəyişən metodlar — `own` yalnız bunlarda məcburidir. */
...
/** Oxu marşrutu — `own` tələb olunmur (gövdə dəyişmir). */
interface ReadRoute extends RouteBase { method: ReadMethod; own?: never }

/**
 * Mutasiya marşrutu — avtorizasiya bəyanı MƏCBURİDİR.
 *
 * Birləşmənin iki qolu var, çünki qəbul olunan iki cavab var: ya deklarativ
 * `perm`, ya da açıq `own`. İkisi də yazılmasa `tsc` sınır və marşrut cədvəli
 * ümumiyyətlə kompilyasiya olunmur — audit məhz bunu istəyirdi ("ikisi də
 * yoxdursa marşrut cədvəli build vaxtı sınsın").
 *
 * ⚠ Hər ikisini birlikdə yazmaq da olar (`perm` + `own: 'handler'`): bəzi
 *   marşrutlarda icazə qapısından SONRA əlavə sahiblik yoxlaması da var.
 */
type MutationRoute = RouteBase & { method: MutationMethod } & (
  | { perm: string; own?: Ownership }
  | { own: Ownership; perm?: string }
);

type Route = ReadRoute | MutationRoute;
...
const ROUTES: Route[] = [
  // auth
  { method: 'POST', pattern: /^\/api\/auth\/register$/, handler: R.register, own: 'public', rl: 'auth' },
  { method: 'POST', pattern: /^\/api\/auth\/login$/, handler: R.login, own: 'public', rl: 'auth' },
  { method: 'POST', pattern: /^\/api\/auth\/logout$/, handler: R.logout, own: 'public', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/auth\/refresh$/, handler: R.refresh, own: 'public', rl: 'refresh' },
  ...
  { method: 'POST', pattern: /^\/api\/auth\/mfa$/, handler: R.mfaVerify, own: 'public', rl: 'auth' },
  { method: 'GET', pattern: /^\/api\/me\/mfa$/, handler: R.mfaStatus, auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/me\/mfa\/setup$/, handler: R.mfaSetup, auth: true, own: 'handler', rl: 'auth' },
  { method: 'POST', pattern: /^\/api\/me\/mfa\/confirm$/, handler: R.mfaConfirm, auth: true, own: 'handler', rl: 'auth' },
  { method: 'POST', pattern: /^\/api\/me\/mfa\/backup-codes$/, handler: R.mfaRegenerateBackup, auth: true, own: 'handler', rl: 'auth' },
  { method: 'DELETE', pattern: /^\/api\/me\/mfa$/, handler: R.mfaDisable, auth: true, own: 'handler', rl: 'auth' },
  ...
  { method: 'POST', pattern: /^\/api\/auth\/magic-link$/, handler: R.magicLinkRequest, own: 'public', rl: 'auth' },
  ...
  { method: 'POST', pattern: /^\/api\/auth\/password-reset$/, handler: R.passwordResetRequest, own: 'public', rl: 'auth' },
  { method: 'POST', pattern: /^\/api\/auth\/password-reset\/confirm$/, handler: R.passwordResetConfirm, own: 'public', rl: 'auth' },
  ...
  { method: 'DELETE', pattern: /^\/api\/me\/oauth\/(github|google|linkedin)$/, handler: R.unlinkOAuth, auth: true, own: 'handler', rl: 'write' },
  // Aktiv sessiyalar / cihazlar (Bənd 3)
  { method: 'GET', pattern: /^\/api\/auth\/sessions$/, handler: R.listSessions, auth: true, rl: 'read' },
  { method: 'DELETE', pattern: /^\/api\/auth\/sessions\/others$/, handler: R.revokeOtherSessions, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/auth\/sessions\/([\w-]+)$/, handler: R.revokeOneSession, auth: true, own: 'handler', rl: 'write' },
  { method: 'GET', pattern: /^\/api\/auth\/username-available$/, handler: R.usernameAvailable, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/auth\/change-password$/, handler: R.changePassword, auth: true, own: 'handler', rl: 'auth' },
  { method: 'POST', pattern: /^\/api\/auth\/change-username$/, handler: R.changeUsername, auth: true, own: 'handler', rl: 'auth' },
  { method: 'DELETE', pattern: /^\/api\/auth\/account$/, handler: R.deleteAccount, auth: true, own: 'handler', rl: 'auth' },
  ...
  { method: 'PATCH', pattern: /^\/api\/me$/, handler: R.patchMe, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/me\/settings$/, handler: R.patchSettings, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/users\/([\w.]+)\/view$/, handler: R.recordProfileView, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PUT', pattern: /^\/api\/follows\/([\w-]+)$/, handler: R.followPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/follows\/([\w-]+)$/, handler: R.followDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/bookmarks\/([\w-]+)$/, handler: R.bookmarkPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/bookmarks\/([\w-]+)$/, handler: R.bookmarkDelete, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/posts$/, handler: R.createPost, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/posts\/([\w-]+)$/, handler: R.patchPost, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)$/, handler: R.deletePost, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/posts\/([\w-]+)\/repost$/, handler: R.toggleRepost, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/like$/, handler: R.likePut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/like$/, handler: R.likeDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'GET', pattern: /^\/api\/posts\/([\w-]+)\/comments$/, handler: R.listComments, auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/posts\/([\w-]+)\/comments$/, handler: R.addComment, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)$/, handler: R.editComment, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)$/, handler: R.deleteComment, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/like$/, handler: R.commentLikePut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/like$/, handler: R.commentLikeDelete, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/reaction$/, handler: R.commentReactionPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/reaction$/, handler: R.commentReactionDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/pin$/, handler: R.commentPin, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/pin$/, handler: R.commentUnpin, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/hide$/, handler: R.commentHide, auth: true, own: 'handler', rl: 'admin' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/hide$/, handler: R.commentRestore, auth: true, own: 'handler', rl: 'admin' },
  { method: 'POST', pattern: /^\/api\/posts\/([\w-]+)\/comments\/([\w-]+)\/report$/, handler: R.commentReport, auth: true, own: 'handler', rl: 'write' },
  // Post reaksiya / moderasiya / şikayət (0040) — şərhlərdəki modelin eynisi.
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/reaction$/, handler: R.postReactionPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/reaction$/, handler: R.postReactionDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/pin$/, handler: R.postPin, auth: true, own: 'handler', rl: 'admin' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/pin$/, handler: R.postUnpin, auth: true, own: 'handler', rl: 'admin' },
  ...
  { method: 'POST', pattern: /^\/api\/posts\/([\w-]+)\/profile-pin$/, handler: R.toggleProfilePin, auth: true, own: 'handler', rl: 'write' },

  /* ═══════════ İŞ SAHƏSİ (Tapşırıqlar) ═══════════
     ⚠ SABİT SEQMENTLƏR PARAMETRLİLƏRDƏN ƏVVƏLDİR: `/tasks/active-timer`
       `/tasks/([\w-]+)` naxışına da uyğun gəlir və parametrli olan öndə
       olsaydı, taymer sorğusu "active-timer" adlı tapşırıq axtarardı. */
  { method: 'GET', pattern: /^\/api\/ws\/tasks$/, handler: R.wsTasks, auth: true, rl: 'read' },
  { method: 'GET', pattern: /^\/api\/ws\/stats$/, handler: R.wsStats, auth: true, rl: 'read' },
  { method: 'GET', pattern: /^\/api\/ws\/trend$/, handler: R.wsTrend, auth: true, rl: 'read' },
  { method: 'GET', pattern: /^\/api\/ws\/meta$/, handler: R.wsMeta, auth: true, rl: 'read' },
  { method: 'GET', pattern: /^\/api\/ws\/timer$/, handler: R.wsTimerActive, auth: true, rl: 'read' },
  { method: 'GET', pattern: /^\/api\/ws\/automations$/, handler: R.wsAutomationList, auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks$/, handler: R.wsCreate, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/bulk$/, handler: R.wsBulk, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/labels$/, handler: R.wsLabelCreate, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/labels\/([\w-]+)$/, handler: R.wsLabelDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/sprints$/, handler: R.wsSprintCreate, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/ws\/sprints\/([\w-]+)$/, handler: R.wsSprintPatch, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/sprints\/([\w-]+)$/, handler: R.wsSprintDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/views$/, handler: R.wsViewSave, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/views\/([\w-]+)$/, handler: R.wsViewDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/automations$/, handler: R.wsAutomationCreate, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/automations\/([\w-]+)$/, handler: R.wsAutomationDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'GET', pattern: /^\/api\/ws\/tasks\/([\w-]+)$/, handler: R.wsDetail, auth: true, rl: 'read' },
  { method: 'PATCH', pattern: /^\/api\/ws\/tasks\/([\w-]+)$/, handler: R.wsUpdate, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/move$/, handler: R.wsMove, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/watch$/, handler: R.wsWatch, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/labels$/, handler: R.wsLabelToggle, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/checklist$/, handler: R.wsCheckAdd, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/checklist\/([\w-]+)$/, handler: R.wsCheckPatch, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/checklist\/([\w-]+)$/, handler: R.wsCheckDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/comments$/, handler: R.wsCommentAdd, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/comments\/([\w-]+)$/, handler: R.wsCommentPatch, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/comments\/([\w-]+)$/, handler: R.wsCommentDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/deps$/, handler: R.wsDepAdd, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/deps\/([\w-]+)$/, handler: R.wsDepDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/timer\/start$/, handler: R.wsTimerStart, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/timer\/stop$/, handler: R.wsTimerStop, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/time$/, handler: R.wsTimeAdd, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/ws\/tasks\/([\w-]+)\/attachments\/([\w-]+)$/, handler: R.wsAttachDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/posts\/([\w-]+)\/hide$/, handler: R.postHide, auth: true, own: 'handler', rl: 'admin' },
  { method: 'DELETE', pattern: /^\/api\/posts\/([\w-]+)\/hide$/, handler: R.postRestore, auth: true, own: 'handler', rl: 'admin' },
  { method: 'POST', pattern: /^\/api\/posts\/([\w-]+)\/report$/, handler: R.postReport, auth: true, own: 'handler', rl: 'write' },
  // Sorğu səsverməsi (0043) — eyni variantı təkrar seçmək səsi götürür.
  { method: 'POST', pattern: /^\/api\/posts\/([\w-]+)\/poll\/vote$/, handler: R.pollVote, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/rooms\/([\w-]+)\/messages$/, handler: R.sendRoomMessage, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)$/, handler: R.editRoomMessage, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)$/, handler: R.deleteRoomMessage, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PUT', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)\/pin$/, handler: R.pinRoomMessage, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)\/pin$/, handler: R.unpinRoomMessage, auth: true, own: 'handler', rl: 'write' },
  // Reaksiya · əlfəcin (0047). Şərh reaksiyaları ilə eyni PUT/DELETE konvensiyası.
  { method: 'PUT', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)\/reaction$/, handler: R.roomReactionPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)\/reaction$/, handler: R.roomReactionDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)\/bookmark$/, handler: R.roomBookmarkPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/rooms\/([\w-]+)\/messages\/([\w-]+)\/bookmark$/, handler: R.roomBookmarkDelete, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/dms\/to\/([\w-]+)$/, handler: R.sendDM, auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)$/, handler: R.editDM, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)$/, handler: R.deleteDMMsg, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/dms\/([\w_-]+)\/read$/, handler: R.markThreadRead, auth: true, own: 'handler', rl: 'write' },
  { method: 'GET', pattern: /^\/api\/dms\/([\w_-]+)\/pins$/, handler: R.listDMPins, auth: true, rl: 'read' },
  { method: 'PUT', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)\/pin$/, handler: R.pinDMMessage, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)\/pin$/, handler: R.unpinDMMessage, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)\/reaction$/, handler: R.dmReactionPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)\/reaction$/, handler: R.dmReactionDelete, auth: true, own: 'handler', rl: 'write' },
  { method: 'PUT', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)\/bookmark$/, handler: R.dmBookmarkPut, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/dms\/([\w_-]+)\/messages\/([\w-]+)\/bookmark$/, handler: R.dmBookmarkDelete, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/messages\/forward$/, handler: R.forwardMessage, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/presence$/, handler: R.heartbeat, auth: true, own: 'handler', rl: 'presence' },
  ...
  { method: 'POST', pattern: /^\/api\/notifications\/mutes$/, handler: R.toggleMute, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/notifications\/read-all$/, handler: R.readAllNotifs, auth: true, own: 'handler', rl: 'write' },
  /* Toplu əməliyyat — ən çox 200 sətir (bax `BULK_MAX`). `write` səbəti
     kifayətdir: seçim rejimi bir düymə basımına BİR sorğu göndərir. */
  { method: 'POST', pattern: /^\/api\/notifications\/bulk$/, handler: R.bulkNotifs, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/notifications\/([\w-]+)\/read$/, handler: R.readNotif, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/notifications\/([\w-]+)$/, handler: R.deleteNotif, auth: true, own: 'handler', rl: 'write' },

  // tasks + submissions
  { method: 'GET', pattern: /^\/api\/tasks$/, handler: R.listTasks, auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/tasks$/, handler: R.createTask, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PUT', pattern: /^\/api\/tasks\/([\w-]+)\/submission$/, handler: R.submitSolution, auth: true, own: 'handler', rl: 'write' },
  ...
  // reports
  { method: 'POST', pattern: /^\/api\/reports$/, handler: R.createReport, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/public\/contact$/, handler: R.contactSubmit, own: 'public', rl: 'form' },
  ...
  { method: 'PUT',  pattern: /^\/api\/users\/([\w-]+)\/role$/, handler: R.setUserRole, auth: true, own: 'handler', rl: 'admin' },
  { method: 'PUT',  pattern: /^\/api\/users\/([\w-]+)\/permission$/, handler: R.setUserPermission, auth: true, own: 'handler', rl: 'admin' },
  ...
  { method: 'POST', pattern: /^\/api\/users\/([\w-]+)\/warn$/, handler: R.warnUser, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/users\/([\w-]+)\/ban$/, handler: R.banUser, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/users\/([\w-]+)\/mute$/, handler: R.muteUser, auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/users\/([\w-]+)\/restore$/, handler: R.restoreUser, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/me\/moderator-application$/, handler: R.applyForModerator, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/me\/moderator-application$/, handler: R.withdrawModeratorApplication, auth: true, own: 'handler', rl: 'write' },
  // ── Dəvət axını — PRD §6 ─────────────────────────────────────────────────
  //
  ...
  { method: 'POST',   pattern: /^\/api\/me\/invites$/, handler: R.createInvite, auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/me\/invites\/([A-Za-z0-9]{1,16})$/, handler: R.revokeInvite, auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/ai\/chat$/, handler: handleChat, auth: true, own: 'handler', rl: 'ai' },
  ...
  { method: 'POST', pattern: /^\/api\/teams$/, handler: async (c) => TR(c, m => m.createTeam(c)), auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PATCH', pattern: /^\/api\/teams\/([\w-]+)$/, handler: async (c, id) => TR(c, m => m.updateTeam(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)$/, handler: async (c, id) => TR(c, m => m.deleteTeam(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/join$/, handler: async (c, id) => TR(c, m => m.joinTeam(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/leave$/, handler: async (c, id) => TR(c, m => m.leaveTeam(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/transfer$/, handler: async (c, id) => TR(c, m => m.transferOwnership(c, id)), auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'PATCH', pattern: /^\/api\/teams\/([\w-]+)\/members\/([\w-]+)$/, handler: async (c, id, uid) => TR(c, m => m.updateMemberRole(c, id, uid)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/members\/([\w-]+)$/, handler: async (c, id, uid) => TR(c, m => m.removeTeamMember(c, id, uid)), auth: true, own: 'handler', rl: 'write' },

  // Roles
  { method: 'GET', pattern: /^\/api\/teams\/([\w-]+)\/roles$/, handler: async (c, id) => TR(c, m => m.getTeamRoles(c, id)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/roles$/, handler: async (c, id) => TR(c, m => m.createTeamRole(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/teams\/([\w-]+)\/roles\/([\w-]+)$/, handler: async (c, id, rid) => TR(c, m => m.updateTeamRole(c, id, rid)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/roles\/([\w-]+)$/, handler: async (c, id, rid) => TR(c, m => m.deleteTeamRole(c, id, rid)), auth: true, own: 'handler', rl: 'write' },

  // Invites
  { method: 'GET', pattern: /^\/api\/invites$/, handler: async (c) => TR(c, m => m.getMyInvites(c)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/invites\/([\w-]+)\/accept$/, handler: async (c, iid) => TR(c, m => m.acceptTeamInvite(c, iid)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/invites\/([\w-]+)\/decline$/, handler: async (c, iid) => TR(c, m => m.declineTeamInvite(c, iid)), auth: true, own: 'handler', rl: 'write' },
  { method: 'GET', pattern: /^\/api\/teams\/([\w-]+)\/invites$/, handler: async (c, id) => TR(c, m => m.getTeamInvites(c, id)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/invites$/, handler: async (c, id) => TR(c, m => m.createTeamInvite(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/invites\/([\w-]+)\/accept$/, handler: async (c, _t, i) => TR(c, m => m.acceptTeamInvite(c, i)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/invites\/([\w-]+)$/, handler: async (c, t, i) => TR(c, m => m.deleteTeamInvite(c, t, i)), auth: true, own: 'handler', rl: 'write' },

  // Projects
  { method: 'GET', pattern: /^\/api\/teams\/([\w-]+)\/projects$/, handler: async (c, id) => TR(c, m => m.getTeamProjects(c, id)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/projects$/, handler: async (c, id) => TR(c, m => m.createTeamProject(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/teams\/([\w-]+)\/projects\/([\w-]+)$/, handler: async (c, t, p) => TR(c, m => m.updateTeamProject(c, t, p)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/projects\/([\w-]+)$/, handler: async (c, t, p) => TR(c, m => m.deleteTeamProject(c, t, p)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/projects\/([\w-]+)\/join$/, handler: async (c, t, p) => TR(c, m => m.joinTeamProject(c, t, p)), auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/projects\/([\w-]+)\/requests\/([\w-]+)\/approve$/, handler: async (c, t, p, r) => TR(c, m => m.approveProjectRequest(c, t, p, r)), auth: true, own: 'handler', rl: 'write' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/projects\/([\w-]+)\/requests\/([\w-]+)\/reject$/, handler: async (c, t, p, r) => TR(c, m => m.rejectProjectRequest(c, t, p, r)), auth: true, own: 'handler', rl: 'write' },

  // Tasks
  { method: 'GET', pattern: /^\/api\/teams\/([\w-]+)\/tasks$/, handler: async (c, id) => TR(c, m => m.getTeamTasks(c, id)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/tasks$/, handler: async (c, id) => TR(c, m => m.createTeamTask(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'PATCH', pattern: /^\/api\/teams\/([\w-]+)\/tasks\/([\w-]+)$/, handler: async (c, t, tk) => TR(c, m => m.updateTeamTask(c, t, tk)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/tasks\/([\w-]+)$/, handler: async (c, t, tk) => TR(c, m => m.deleteTeamTask(c, t, tk)), auth: true, own: 'handler', rl: 'write' },

  // Files
  { method: 'GET', pattern: /^\/api\/teams\/([\w-]+)\/files$/, handler: async (c, id) => TR(c, m => m.getTeamFiles(c, id)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/files$/, handler: async (c, id) => TR(c, m => m.recordTeamFile(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/files\/([\w-]+)$/, handler: async (c, t, f) => TR(c, m => m.deleteTeamFile(c, t, f)), auth: true, own: 'handler', rl: 'write' },
  ...
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/(?:feed|posts)$/, handler: async (c, id) => TR(c, m => m.createTeamPost(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/(?:feed|posts)\/([\w-]+)$/, handler: async (c, t, p) => TR(c, m => m.deleteTeamPost(c, t, p)), auth: true, own: 'handler', rl: 'write' },

  // Chat rooms
  { method: 'GET', pattern: /^\/api\/teams\/([\w-]+)\/rooms$/, handler: async (c, id) => TR(c, m => m.getTeamRooms(c, id)), auth: true, rl: 'read' },
  { method: 'POST', pattern: /^\/api\/teams\/([\w-]+)\/rooms$/, handler: async (c, id) => TR(c, m => m.createTeamRoom(c, id)), auth: true, own: 'handler', rl: 'write' },
  { method: 'DELETE', pattern: /^\/api\/teams\/([\w-]+)\/rooms\/([\w-]+)$/, handler: async (c, t, r) => TR(c, m => m.deleteTeamRoom(c, t, r)), auth: true, own: 'handler', rl: 'write' },
  ...
  // upload
  { method: 'POST', pattern: /^\/api\/upload$/, handler: R.upload, auth: true, own: 'handler', rl: 'upload' },
];
